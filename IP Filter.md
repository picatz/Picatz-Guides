...
**If you do not want the filtering rules loaded at boot-time, put the ipf.conf file in a location of your choice 
  then activate with "ipf" command
...
role
	Specifies the role of the pool in Oracle Solaris IP Filter. The only role you can reference is "ipf".
type
	Specifies the storage format of the pool.
number
	Specifies the reference number that is used by the filtering rule.
----------------------------------------------------------------------------------------------------------
EX. Reference the group of addresses 10.1.1.1 and 10.1.1.2, and the network 192.16.1.0 as pool number 13, you 
	would include the following rule in the address pool config. file:
table role = ipf type = tree number = 13
{ 10.1.1.1/32, 10.1.1.2/32, 192.168.1.0/24 };

Then, to reference the pool number 13 in a filtering rule, you would construct the rule similar to the following example:
pass in from pool/13 to any
**you must load the pool file before loading the rules files that contains the reference to the pool. If you don't, the pool is
	undefined, as shown in the following output:
# ipfstat -io
...
-ipf(4)
	Containst he grammar and syntax for creating Oracle Solaris IP filter packet filtering rules.
-ipfilter(5)
